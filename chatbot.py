...
best_prec1 = 0
saved_model = "C:/Users/Niranjan khedkar/OneDrive/Desktop/leafnet with model/leafnet/saved_models/model_best_augment.pth.tar"
...
def test(test_loader, model, criterion, classes):
    ...
    for i, (input, target) in enumerate(test_loader):
        ...
        output = model(input_var).detach().cpu().numpy()

        # confidences, predicted = torch.max(output.data, 1)
        # labels = classes[predicted.item()]
        # confidences = int(confidences.item())
    
        yhat=output[0]
        # yhat = np.array(torch.nn.Softmax(dim=0)(output.data))[0]
        yhat[yhat<0.3] = 0.
        print(yhat)
...
def predict_leaf(pic_url):
    ...
    return labels, confidences
def main():
    labels, confidences = predict_leaf('https://th.bing.com/th/id/OIP.CCOy5QTU2K2QLUKua6FQZgHaE6?pid=ImgDet&rs=1')
